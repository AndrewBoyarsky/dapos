...
//        ValidatorControlAttachment attachment = tx.getAttachment(ValidatorControlAttachment.class);
        ValidatorEntity entity = validatorService.get(tx.getRecipient());
        if (entity == null) {
            throw new TxNotValidException("Validator " + tx.getRecipient() + " was not found", null, tx, ErrorCodes.NOT_FOUND_VALIDATOR);
        }
